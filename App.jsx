...
const parseAgentTopics = content => {
  ...
  // XML format: <apiName>TopicName</apiName> inside <topics> block
  for (const m of content.matchAll(/<apiName>([^<]+)<\/apiName>/g)) results.add(m[1].trim());
  // XML format: <masterLabel>TopicName</masterLabel>
  for (const m of content.matchAll(/<masterLabel>([^<]+)<\/masterLabel>/g)) results.add(m[1].trim());
  // XML format: <name>TopicName</name> (inside topic elements)
  for (const m of content.matchAll(/<name>([^<]+)<\/name>/g)) results.add(m[1].trim());
  // Plain text format: "topic TopicName {"
  for (const m of content.matchAll(/^\s*topic\s+([A-Za-z0-9_]+)\s*\{/gm)) results.add(m[1].trim());
  // Plain text format: developer_name: TopicName (only one per topic block)
  if (results.size === 0) {
    for (const m of content.matchAll(/developer_name:\s*["']?([A-Za-z0-9_]+)["']?/g)) results.add(m[1].trim());
  }
  return [...results];
};
...
function Badge({ children, color = "brand" }) {
  const colors = {
    ...
    neutral: { bg: "#f3f3f3",     text: C.textWeak },
  };
...
function Alert({ type = "info", children }) {
  const map = {
    info:    { bg: C.brandLight,  border: C.brand,   color: C.brandDark, icon: "ℹ️" },
...
function YamlViewer({ code, maxHeight = "400px" }) {
  ...
  return (
    <div style={{ background: "#f6f8fa", border: `1px solid ${C.border}`, borderRadius: C.radius,
...
function CmdBlock({ label, cmd, note, serverOnline }) {
  ...
  return (
    <div style={{ border: `1px solid ${C.border}`, borderRadius: C.radius, overflow: "hidden", marginBottom: "8px" }}>
      ...
      <div style={{ padding: "10px 12px", fontFamily: "monospace", fontSize: "11px", color: C.text,
                    whiteSpace: "pre-wrap", wordBreak: "break-all", background: "#f6f8fa" }}>{cmd}</div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
        ...
        <button onClick={load} style={{ background: "none", border: "none", color: C.brand, fontSize: "12px", cursor: "pointer" }}>
          {loading ? <Spinner size={10} /> : "↻ Refresh"}
        </button>
      </div>
      {error && <Alert type="error">{error}</Alert>}
      {!loading && !error && files.length === 0 && (
        <div style={{ padding: "20px", textAlign: "center", background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: C.radius, color: C.textWeak, fontSize: "13px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
        {files.map(f => {
          ...
          const topics = parseTopics(f.content);
          const modified = new Date(f.modified).toLocaleDateString("fr-FR", { day: "2-digit", month: "short" });
          return (
            <div key={f.name} onClick={() => onSelect(f)}
              style={{ padding: "10px 14px", background: isSelected ? C.brandLight : C.surface,
                       border: `1px solid ${isSelected ? C.brand : C.border}`, borderRadius: C.radius,
                       cursor: "pointer", transition: "all 0.15s", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: "13px", fontWeight: "600", color: isSelected ? C.brandDark : C.text, fontFamily: "monospace" }}>
                  {isSelected ? "✓ " : ""}{f.name}
                </div>
                <div style={{ fontSize: "11px", color: C.textWeak, marginTop: "2px" }}>
                  {topics.length > 0 && `${topics.length} topic${topics.length > 1 ? "s" : ""}`}
                  {topics.length > 0 && " · "}
                  {(f.size / 1024).toFixed(1)} KB
                </div>
              </div>
              <span style={{ fontSize: "11px", color: C.textWeaker }}>{modified}</span>
            </div>
          );
        })}
      </div>
            ...
            <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              {aiEvalMode === "pick" && (
                <FilePicker
                  ...
                  emptyMsg="No AiEvaluationDefinition XML files found in your project."
                  serverOnline={serverOnline}
                />
              )}
...
export default function App() {
  ...
  const NavBtn = ({ id, icon, label }) => (
    <button onClick={() => setPage(id)}
      style={{ padding: "10px 20px", background: page === id ? C.brand : "transparent",
               color: page === id ? "#fff" : C.textWeak, border: "none",
               borderBottom: `3px solid ${page === id ? C.brand : "transparent"}`,
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Salesforce Sans', 'Inter', 'DM Sans', system-ui, sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ::-webkit-scrollbar { width: 5px; } ::-webkit-scrollbar-track { background: #f3f3f3; } ::-webkit-scrollbar-thumb { background: #dddbda; border-radius: 3px; }
        ...
        code { background: #f3f3f3; padding: 1px 5px; border-radius: 3px; font-family: monospace; font-size: 0.9em; }
      `}</style>
      ...
      <div style={{ background: C.brand, padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px 0" }}>
          ...
          <span style={{ color: "#fff", fontWeight: "700", fontSize: "16px" }}>TDAD</span>
          <span style={{ color: "rgba(255,255,255,0.6)", fontSize: "13px" }}>for Agentforce</span>
        </div>
        {/* Server status */}
      ...
      {/* Nav */}
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "0 24px", display: "flex", gap: "0", boxShadow: C.shadow }}>
        <NavBtn id="agent" icon="🤖" label="Agent Spec" />
        <NavBtn id="test"  icon="🧪" label="Test Spec" />
      </div>

      {/* Offline banner */}
      {status === false && (
        <div style={{ background: "#fff3cd", borderBottom: "1px solid #ffc107", padding: "10px 24px", display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
          <span style={{ fontSize: "13px", color: "#856404" }}>⚠️ Local server not running — AI generation and file operations are unavailable.</span>
          <code style={{ fontSize: "12px", background: "#f8d7da", color: "#842029", padding: "2px 8px", borderRadius: "4px" }}>node tdad-server.js --project /path/to/sfdx-project</code>
        </div>
      )}
