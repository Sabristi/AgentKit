...
const METRICS = [
  ...
  { id: "output_latency_milliseconds", label: "Latency (ms)",    desc: "Latency in ms from request to response" },
...
function PageTestSpec({ serverOnline, targetOrg = "my-dev-org", projectName = "" }) {
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {runs.map((run, idx) => {
                ...
                // Fix loop: show below the run it was triggered by (next/older run)
...
function TraceAnalysisPanel({ tracesPath, serverOnline }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/preview/traces?path=${encodeURIComponent(tracesPath)}`)
      ...
      .then(d => {
        ...
        const out = (d.files || []).map(f => {
          try {
            const trace = JSON.parse(f.content);
            const steps = trace.plan || [];

            const utterance = steps.find(s => s.type === "UserInputStep")?.message || "";

            // Topic — TransitionStep to_agent
            const transitions = steps
              .filter(s => s.type === "TransitionStep")
              .map(s => s.data?.to_agent).filter(Boolean);

            // Actions invoked — from response_messages in LLMStep
            const actions = steps
              .filter(s => s.type === "LLMStep")
              .flatMap(s => s.response_messages || [])
              .filter(m => m.tool_invocation)
              .map(m => m.tool_invocation.name)
              .filter(n => n && !n.startsWith("go_") && !n.startsWith("back_") && !n.startsWith("escalate_"));

            const allActionCalls = [...new Set(actions)];

            // Tools visible (EnabledToolsStep) — filter out routing tools and guardrails
            const enabledTools = steps
              .filter(s => s.type === "EnabledToolsStep")
              .flatMap(s => s.data?.enabled_tools || [])
              .filter(t => !t.includes("Guardrail") && !t.includes("Content") && !t.includes("Injection") && !t.includes("Engineering"));

            // Final agent response
            const finalMsg = steps
              .filter(s => s.type === "PlannerResponseStep")
              .slice(-1)[0]?.message || "";

            // Safety score
            const safety = steps
              .filter(s => s.type === "PlannerResponseStep")
              .slice(-1)[0]?.safetyScore?.safetyScore?.safety_score;

            // Grounding
            const grounding = steps
              .filter(s => s.type === "ReasoningStep")
              .slice(-1)[0]?.category;

            // Latency
            const times = steps.map(s => s.endExecutionTime).filter(Boolean);
            const starts = steps.map(s => s.startExecutionTime).filter(Boolean);
            const latency = times.length && starts.length ? Math.max(...times) - Math.min(...starts) : null;

            return { name: f.name, utterance, transitions, allActionCalls, enabledTools, finalMsg, safety, grounding, latency };
          } catch(e) {
            return { name: f.name, error: e.message };
          }
        });
        setParsed(out);
      })
  ...
  const locked = !tracesPath;

  return (
    <div style={{ border:`1px solid ${C.border}`, borderRadius:C.radius, overflow:"hidden", marginTop:"4px",
                  opacity:locked ? 0.45 : 1, pointerEvents:locked ? "none" : "auto", transition:"opacity 0.2s" }}>
      <div style={{ padding:"10px 14px", background:C.surfaceAlt, borderBottom:`1px solid ${C.border}`,
                    display:"flex", alignItems:"center", gap:"10px" }}>
        <div style={{ width:"22px", height:"22px", borderRadius:"50%",
                      border:`2px solid ${locked ? C.border : C.gold}`,
                      display:"flex", alignItems:"center", justifyContent:"center",
                      fontSize:"10px", fontWeight:"700", color:locked ? C.textWeaker : C.gold }}>
          {locked ? "—" : "4"}
        </div>
        <span style={{ fontSize:"12px", fontWeight:"600", color:locked ? C.textWeaker : C.text, flex:1 }}>Trace Analysis</span>
        {locked && <span style={{ fontSize:"11px", color:C.textWeaker }}>Run step 3 first</span>}
        {!loading && parsed.length > 0 && (
          <span style={{ fontSize:"11px", color:C.textWeak }}>{parsed.length} utterance{parsed.length !== 1 ? "s" : ""}</span>
        )}
      </div>

      <div style={{ padding:"12px 14px" }}>
        {loading && <div style={{ color:C.textWeak, fontSize:"12px", display:"flex", alignItems:"center", gap:"6px" }}><Spinner size={11} /> Loading traces…</div>}
        {error && <Alert type="error">{error}</Alert>}
        ...
        {!loading && !error && tracesPath && parsed.length === 0 && !error && (
        ...
        {parsed.map((item, idx) => {
          ...
          return (
            <div key={idx} style={{ marginBottom:"8px", border:`1px solid ${C.border}`, borderRadius:C.radius, overflow:"hidden" }}>
              <div onClick={() => setExpanded(isOpen ? null : idx)}
                style={{ padding:"8px 12px", background:C.surface, cursor:"pointer",
                         display:"flex", alignItems:"center", gap:"10px" }}>
                <span style={{ fontSize:"11px", color:C.textWeaker, fontWeight:"700", flexShrink:0 }}>#{idx+1}</span>
                <div style={{ flex:1 }}>
                  <span style={{ fontSize:"12px", color:C.text, fontStyle:"italic" }}>
                    "{item.utterance?.substring(0,60)}{item.utterance?.length > 60 ? "…" : ""}"
                  </span>
                </div>
                {item.transitions?.length > 0 && (
                  <span style={{ fontSize:"11px", fontWeight:"600", color:C.brand, fontFamily:"monospace" }}>
              ...
              {isOpen && (
                <div style={{ borderTop:`1px solid ${C.border}`, padding:"14px", display:"flex", flexDirection:"column", gap:"12px" }}>
                  {item.error ? <Alert type="error">Parse error: {item.error}</Alert> : (<>

                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px" }}>
                      <TraceInfoBlock label="Topic routing" icon="🗺">
                        {item.transitions?.length > 0
                          ? item.transitions.map((t,i) => (
                              <span key={i} style={{ display:"inline-block", fontSize:"11px", fontWeight:"600",
                                                     color:C.brand, background:C.brandLight, borderRadius:"4px",
                                                     padding:"2px 8px", marginRight:"4px", marginBottom:"2px" }}>{t}</span>
                            ))
                          : <span style={{ color:C.textWeaker, fontSize:"11px" }}>No transition — stayed in greet_and_route</span>
                        }
                      </TraceInfoBlock>

                      <TraceInfoBlock label="Tools visible to planner" icon="🔧">
                        <span style={{ fontSize:"11px", color:C.textWeak, fontFamily:"monospace", lineHeight:"1.7" }}>
                          {item.enabledTools?.length > 0 ? item.enabledTools.join(" · ") : "—"}
                        </span>
                      </TraceInfoBlock>
                    </div>

                    <TraceInfoBlock label="Actions invoked" icon="⚡">
                      {item.allActionCalls?.length > 0
                        ? item.allActionCalls.map((a,i) => (
                            <span key={i} style={{ display:"inline-block", fontSize:"11px", fontWeight:"600",
                                                   color:"#3fb950", background:"rgba(63,185,80,0.1)", borderRadius:"4px",
                                                   padding:"2px 8px", marginRight:"4px" }}>{a}</span>
                          ))
                        : <span style={{ color:C.textWeaker, fontSize:"11px" }}>No action invoked — routing or FAQ only</span>
                      }
                    </TraceInfoBlock>

                    <TraceInfoBlock label="Agent response" icon="💬">
                      <span style={{ fontSize:"12px", color:C.textWeak, lineHeight:"1.6" }}>
                        {item.finalMsg || "—"}
                      </span>
                    </TraceInfoBlock>
                    ...
                    <div style={{ display:"flex", gap:"10px", flexWrap:"wrap" }}>
                      {item.grounding && (
                        <TracePill label="Grounding" value={item.grounding}
                          color={item.grounding === "GROUNDED" ? C.success : C.warning} />
                      )}
                      {item.safety !== undefined && item.safety !== null && (
                        <TracePill label="Safety" value={item.safety.toFixed(2)}
                          color={item.safety >= 0.9 ? C.success : C.error} />
                      )}
                      {item.latency !== null && item.latency !== undefined && (
                        <TracePill label="Latency" value={item.latency + " ms"}
                          color={item.latency < 3000 ? C.success : item.latency < 6000 ? C.warning : C.error} />
                      )}
                    </div>
...
function TraceInfoBlock({ label, icon, children }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"6px" }}>
...
function TracePill({ label, value, color }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:"6px", padding:"4px 10px",
                  background:color + "18", border:`1px solid ${color}33`, borderRadius:"999px" }}>
      <span style={{ fontSize:"10px", color:C.textWeaker, fontWeight:"600", textTransform:"uppercase" }}>{label}</span>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Salesforce Sans', 'Inter', 'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, overflowX: "auto" }}>
        <div style={{ padding: "0 clamp(20px,4vw,56px)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        {[
      ...
      <div style={{ padding: "28px clamp(20px, 4vw, 56px) 80px" }}>
        ...
        {page === "test"         && <PageTestSpec      serverOnline={serverOnline} targetOrg={targetOrg} projectName={projectName} />}
